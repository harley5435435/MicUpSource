    ...
    if SCRIPT_EXECUTED or getgenv().SCRIPT_EXECUTED and not _G.SCRIPT_EXECUTED == true then  
        return Notification:Notify("Heads Up!", "Harley Simple Hub is already running!", 5)
    end
    ...
    elseif enabled_vc == false and game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        return Notification:Notify("Sorry!", "Unable to load Harley Simple Hub (No VC)", 5)
    else
    ...
    print("4")
    local watchedPlayers = {"3", "1"}
    local specialPlayer = "2"
    local support_team = {"4", "5", "6", "7", "8"}
    local all_beta_testers = {"9", "0", "0"}
    ...
    local function make_title(player, text, color, transparency)
        local function applyToCharacter(character)
            ...
            if color == Color3.fromRGB(255, 255, 255) then
                ...
                if head:FindFirstChild("HarleySimpleHub") then return end

                local billboardGui = Instance.new("BillboardGui")
                billboardGui.Name = "HarleySimpleHub"
    ...
    local function assign(player)
        if isWatchedPlayer(player.Name) then
            make_title(player, "👑  Harley Simple Hub | KING 👑", Color3.fromRGB(196, 40, 28), 0)
        elseif player.Name == specialPlayer then
            make_title(player, "👧 Harley Simple Hub | Daughter 👧", Color3.fromRGB(89, 34, 89), 0.3)
        elseif isSupportTeam(player.Name) then
            make_title(player, "⚒️ Harley Simple Hub | Support ⚒️", Color3.fromRGB(52, 142, 64), 0.1)
        elseif beta_tester(player.Name) then
            make_title(player, "✨ Harley Simple Hub | Beta Tester ✨", Color3.fromRGB(255, 255, 255), 0.1)
        end
    end
    ...
    if not getgenv().maps_loaded or getgenv().maps_loaded == false then
        local function insert_id_asset(asset_ID, Parent)
            ...
            if mapModel and mapModel:IsA("Model") then
                ...
                warn("Failed to load and insert Harley Simple Hub | Crossroads Map Model.")
            end
        end
        wait()
        print("16")
    ...
    if getgenv().has_checked_funcs then
        ...
        getgenv().checkNecessaryFunctions = function(function_checked)
            if function_checked then
                print("[Harley_Simple_Hub::OUTPUT_HOOKED]: "..tostring(function_checked).." ||| Success ✅")
            elseif not function_checked or function_checked == nil then
                warn("[Harley_Simple_Hub::OUTPUT_HOOKED]: "..tostring(function_checked).." ||| Failure ❌")
            else
                warn("[Harley_Simple_Hub::OUTPUT_HOOKED]: "..tostring(function_checked).." ||| Unknown ❓")
            end
        end
